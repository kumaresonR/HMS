...
import DispatchApiService from "../../helpers/services/front-office/front-office-api-service";
...
const DispatchList: React.FC = () => {
        ...
        <ul className="list-inline hstack gap-2 mb-0">
          <li className="list-inline-item">
            <Link
              ...
              onClick={e => { e.preventDefault(); editDispatch(cell.row.original); }}
              to="#" title="Edit"
            >
          ...
          <li className="list-inline-item" title="Delete">
            <Link
              ...
              onClick={e => { e.preventDefault(); deleteDispatch(cell.row.original.id); }}
              to="#"
            >
  ...
  const getAllDispatches = async () => {
    // try {
    //   const result = await dispatchApiService.getAllDispatches();
  ...
  const handleDelete = async () => {
    ...
    // try {
    //   await dispatchApiService.deleteDispatch(selectedId);
  ...
  const editDispatch = (dispatchRecord: any) => {
    navigate('/main/editDispatch', { state: { data: dispatchRecord } });
  };
  ...
  return (
    <>
      <ExportCSVModal show={isExportCSV} onCloseClick={() => setIsExportCSV(false)} data={dispatchData} />
      <Container fluid>
        <FormHeader
          title="Dispatch List"
          pageTitle="Front Office"
          onMinimize={() => reduxDispatch(minimizePage({ route: location.pathname, pageName: 'Dispatch List' }))}
        />
        <Row>
          <Col lg={12}>
            <Card id="dispatchList">
              <CardHeader className="border-0">
                <Row className="g-4 align-items-center">
                  <div className="col-sm">
                    <h5 className="card-title mb-0">Dispatch List</h5>
                  </div>
                  <div className="col-sm-auto">
                    <Button color="primary" onClick={() => setIsExportCSV(true)}>
                      <i className="ri-file-download-line align-bottom me-1"></i> Export
                    </Button>
                    <Link to="/main/addDispatch">
                      <Button color="primary" className="ms-2">
                        <i className="ri-add-fill me-1 align-bottom"></i> Add Dispatch
                      </Button>
                    </Link>
