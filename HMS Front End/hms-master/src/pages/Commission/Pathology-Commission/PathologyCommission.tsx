...
import EditPathologyCommission from './EditPathologyCommission';
import AddPathologyCommission from './AddPathologyCommission';

const PathologyCommission = () => {
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await billingApiService.deletePathologyCommission(selectedId);
    ...
    const editPathologyCommission = (id: any) => {
        navigate('/main/editPathologyCommission', { state: { id: id } })
    }

    const getAllCommission = async () => {
        try {
            let result = await billingApiService.getAllPathologyCommission();
                        ...
                        <li className="list-inline-item">
                            <Link
                                ...
                                onClick={(e) => {
                                    ...
                                    showModal({
                                        content: (
                                            <EditPathologyCommission data={cell.row.original} refresh={getAllCommission} />
                                        ),
                                        title: "Edit Pathology Commission",
                                        size: "lg",
                                    })
                                }}
                                title="Edit"
    ...
    return (
        <React.Fragment>
            <div>
                <Container fluid>
                    <FormHeader title="Pathology Commission"
                        pageTitle="Pathology Commission"
                        onMinimize={() => dispatch(minimizePage({
                            route: location.pathname,
                            pageName: "Pathology Commission",
                        }))} />
                    <Row>
                        <Col lg={12}>
                            <Card id="pathologyCommission">
                                <CardHeader className="border-0">
                                    <Row className="g-4 align-items-center">
                                        <div className="col-sm">
                                            <div>
                                                <h5 className="card-title mb-0">Pathology Commission</h5>
                                            </div>
                                        </div>
                                                ...
                                                <RoleBasedComponent allowedRoles={["SUPERADMIN", "ADMIN", "ACCOUNTANT"]}>
                                                    <Button
                                                        onClick={() =>
                                                            showModal({
                                                                content: (
                                                                    <AddPathologyCommission refresh={getAllCommission} />
                                                                ),
                                                                title: "Add Pathology Commission",
                                                                size: "lg",
                                                            })
                                                        }
                                                    ...
                                                    >
                                                        <i className="ri-add-fill me-1 align-bottom"></i> Add Pathology Commission
                                                    </Button>
                                                </RoleBasedComponent>
                                            </div>
                                        </div>
...
export default PathologyCommission;