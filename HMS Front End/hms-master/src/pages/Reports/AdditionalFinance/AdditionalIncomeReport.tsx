...
const AdditionalIncomeReport = () => {
    const reportApiService: ReportApiService = new ReportApiService();

    const [incomeReportData, setAdditionalIncomeReportData] = useState([]);
    const [timeDuration, setTimeDuration] = useState('');
    const [timeDurationValidationError, setTimeDurationValidationError] = useState(false);
    const [startDate, setStartDate] = useState('');
    const [endDate, setEndDate] = useState('');
    const totalAmount = incomeReportData.reduce((sum: any, data: any) => sum + (data.amount || 0), 0);
    ...
    const getAllAdditionalIncomeData = async () => {
        try {
            if (!timeDuration) {
            ...
            let result = await reportApiService.getAllAdditionalIncomeReport(url);
            setAdditionalIncomeReportData(result.data);
            return result.data || [];
        } catch (error: any) {
    ...
    const exportData = async () => {
        try {
            ...
            const result = await reportApiService.exportAdditionalIncomeReport(url); 
            ...
            link.href = objectURL;
            link.download = 'AdditionalIncomeReport.xlsx';
    ...
    return (
        <React.Fragment>
            <div>
                <div>
                    <h5 className="card-title mb-4">Additional Income Report</h5>
                </div>
                <Row>
                    <Col lg={4}>
                        <div className="mb-3">
                            ...
                            <CardHeader className="border-0">
                                <Row className="g-4">
                                    ...
                                    <div className="col-sm-auto">
                                        <div>
                                            <button type="button" onClick={getAllAdditionalIncomeData}
...
export default AdditionalIncomeReport;
