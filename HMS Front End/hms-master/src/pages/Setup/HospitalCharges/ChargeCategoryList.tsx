...
const ChargeCategoryList = () => {
    ...
    const chargeCategoryColumns = [
        {
            header: 'Charge Type',
            accessorKey: 'chargeType.chargeType',
                            ...
                            <>
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteChargesCategory(cell.row.original.categoryId);
                                        }}
                                        to="#"
                                    >
    ...
    const handleApprove = async () => {
        if (selectedBedId) {
            try {
                ...
                await setupApiService.approveChargesCategory(selectedBedId, url);
                toast.success('Charge Category been approved Successfully', { containerId: 'TR' });
                await getAllChargesCategory();
    ...
    const editCharges = (data: any) => {
        navigate('/main/editChargeCategory', { state: { data: data } })
    }

    const deleteChargesCategory = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteChargesCategory(selectedId);
    ...
    const getAllChargesCategory = async () => {
        try {
            let result = await setupApiService.getAllChargesCategory();
                ...
                <Row className="d-flex justify-content-center">
                    <Col xl={12} className="p-0">
                        <Card>
                            ...
                            <Row>
                                <Col lg={12}>
                                    <Card id="chargeCategoryList">
                                        <CardHeader className="border-0">
                                            <Row className="g-4 align-items-center">
                                                <div className="col-sm">
                                                    <div>
                                                        <h5 className="card-title mb-0">Charge Category List</h5>
                                                    </div>
                                                </div>
                                                <div className="col-sm-auto">
                                                    <div>
                                                        <Link to='/main/addChargeCategory'>
                                                            <Button
                                                                size="sm"
                                                                color="primary"
                                                                className="btn btn-primary add-btn me-3"
                                                            >
                                                                <i className="ri-add-fill me-1 align-bottom"></i> Add New Charge Category
                                                            </Button>
                                                        </Link>
                                                        <Button
                                                            type="button"
