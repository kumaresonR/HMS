...
const EditHospitalCharges = () => {
    ...
    const [isTpaDataLoaded, setIsTpaDataLoaded] = useState(false);
    ...
    const editCharges = async () => {
        try {
            const scheduleCharges = tpaData.map((item: any) => ({
                scheduleChargeId: item.scheduleChargeId || null,
                id: item.id,
            ...
            await setupApiService.editCharges(data.chargeId,payload);
            navigate('/main/hospitalMain');
            toast.success('Charge Updated Successfully', { containerId: 'TR' });
        } catch (error: any) {
    ...
    const setData = (data:any) => {
        ...
        setChargeCategory(data.categoryId);
        setUnitType(data.unitTypeId);
        setTaxCategory(data.taxCategoryId);
        ...
        if (data.scheduleCharges) {
            const updatedTpaData = tpaData.map((tpaItem: any) => {
                const matchedScheduleCharge = data.scheduleCharges.find(
                    (scheduleCharge: any) => scheduleCharge.id === tpaItem.id
                );
        
                if (matchedScheduleCharge) {
                    return {
                        ...tpaItem,
                        charge: matchedScheduleCharge.charge,
                    };
                }
    ...
    useEffect(() => {
        if (isTpaDataLoaded && state?.data) {
            setData(state?.data);
        }
    }, [isTpaDataLoaded, state?.data]);
                                        ...
                                        <Row>
                                            <Col lg={8}>
                                                <Row>
                                                    ...
                                                    <Col md={6}>
                                                        <FormGroup>
                                                            <Label for="chargeName">Charge Name <span className='text-danger'> * </span></Label>
                                                            <Input
                                                                type="text"
                                                                id="chargeName"
                                                                value={chargeName}
