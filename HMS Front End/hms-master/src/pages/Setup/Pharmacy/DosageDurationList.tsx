...
const DosageDurationList = () => {
                            ...
                            <>
                                <li className="list-inline-item">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            editDuration(cell.row.original)
                                        }}
                                        title="Edit"
                                    >
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteDoseDuration(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedDurationId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveDoseDuration(selectedDurationId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveDoseDuration(selectedDurationId);
    ...
    const editDuration = (data: any) => {
        navigate('/main/editDosageDuration', { state: { data: data } })
    }

    const deleteDoseDuration = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteDoseDuration(selectedId);
    ...
    const getAllDoseDuration = async () => {
        try {
            let result = await setupApiService.getAllDoseDurationTw();
                    ...
                    <Row className="d-flex justify-content-center">
                        <Col xl={12} className="p-0">
                            <Card>
                                ...
                                <Row>
                                    <Col lg={12}>
                                        <Card id="dosageDurationList">
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    <div className="col-sm">
                                                        <div>
                                                            <h5 className="card-title mb-0">Dosage Duration List</h5>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addDosageDuration'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
