...
const MedicineGroupList = () => {
    ...
    const [selectedMedicineGroupId, setSelectedMedicineGroupId] = useState('');
    ...
    const medicineGroupColumns = [
        {
            header: 'Medicine Group',
            accessorKey: 'medicineGroupName',
                            ...
                            <>
                                <li className="list-inline-item">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            editMedicineGroup(cell.row.original)
                                        }}
                                        title="Edit"
                                    >
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteMedicineGroup(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
    ...
    const doApprove = (id: any, status: any) => {
        setSelectedMedicineGroupId(id);
        setStatus(status);
        setApproveModal(true);
    }

    const handleApprove = async () => {
        if (selectedMedicineGroupId) {
            setStatusLoading(true);
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveMedicineGroup(selectedMedicineGroupId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveMedicineGroup(selectedMedicineGroupId);
                    toast.success('Record has been approved UnAuthorized Successfully', { containerId: 'TR' });
                }
                await getAllMedicineGroup();
    ...
    const editMedicineGroup = (data: any) => {
        navigate('/main/editMedicineGroup', { state: { data: data } })
    }

    const deleteMedicineGroup = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteMedicineGroup(selectedId);
    ...
    const getAllMedicineGroup = async () => {
        try {
            let result = await setupApiService.getAllMedicineGroupTw();
            setMedicineGroupData(result);
        } catch (error: any) {
            return ErrorHandler(error)
        }
    }
                    ...
                    <Row className="d-flex justify-content-center">
                        <Col xl={12} className="p-0">
                            <Card>
                                ...
                                <Row>
                                    <Col lg={12}>
                                        <Card id="medicineGroupList">
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    <div className="col-sm">
                                                        <div>
                                                            <h5 className="card-title mb-0">Medicine Group List</h5>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addMedicineGroup'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
                                                                    className="btn btn-primary add-btn me-3"
                                                                >
                                                                    <i className="ri-add-fill me-1 align-bottom"></i> Add New Group
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                type="button"
