...
const EditRole = () => {
  ...
  const editRole = async () => {
    try {
      ...
      await setupApiService.editRole(data.roleId, payload);
      navigate('/main/humanResourceMainSetup');
      toast.success('Role Updated Successfully', { containerId: 'TR' });
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    if (state?.data) {
      ...
      if (data.scopes) {
        setSelectedScopeName(state?.data?.scopes.map((scope: any) => ({
  ...
  return (
    <React.Fragment>
      <div>
        <Container fluid>
          <FormHeader title="Edit Role" pageTitle="Setup" />
          <Card>
            <CardBody>
