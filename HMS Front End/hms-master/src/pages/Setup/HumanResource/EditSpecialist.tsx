...
const EditSpecialist = () => {
    ...
    const editSpecialist = async () => {
        try {
            const payload: any = {
                // modNo: modNo,
                name: name,
            }
            await setupApiService.editSpecialist(data.specialistId,payload);
            navigate('/main/humanResourceMainSetup');
            toast.success('Specialist Updated Successfully', { containerId: 'TR' });
        } catch (error: any) {
    ...
    return (
        <React.Fragment>
            <div>
                <Container fluid>
                    <FormHeader title="Edit Specialist" pageTitle="Setup" />
                    <Card>
                        <CardBody>
