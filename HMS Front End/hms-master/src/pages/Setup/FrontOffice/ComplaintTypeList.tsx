...
const ComplaintTypeList = () => {
                            ...
                            <>
                                <li className="list-inline-item">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            editComplainType(cell.row.original);
                                        }}
                                        title="Edit"
                                    >
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteComplainType(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
    ...
    const handleApprove = async () => {
        if (selectedComplainId) {
            try {
                ...
                await setupApiService.approveComplainType(selectedComplainId, url);
                toast.success('Record has been approved Successfully', { containerId: 'TR' });
                await getAllComplainType();
    ...
    const editComplainType = (data: any) => {
        navigate('/main/editComplainType', { state: { data: data } })
    }

    const deleteComplainType = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteComplainType(selectedId);
    ...
    const getAllComplainType = async () => {
        try {
            let result = await setupApiService.getAllComplainTypeTw();
                ...
                <Row className="d-flex justify-content-center">
                    <Col xl={12} className="p-0">
                        <Card>
                            ...
                            <Row>
                                <Col lg={12}>
                                    <Card id="complaintTypeList">
                                        <CardHeader className="border-0">
                                            <Row className="g-4 align-items-center">
                                                <div className="col-sm">
                                                    <div>
                                                        <h5 className="card-title mb-0">Complaint Type List</h5>
                                                    </div>
                                                </div>
                                                <div className="col-sm-auto">
                                                    ...
                                                    <div>
                                                        <Link to='/main/addComplainType'>
                                                            <Button

                                                                color="primary"
                                                                className="btn btn-primary add-btn me-3"
                                                            >
                                                                <i className="ri-add-fill me-1 align-bottom"></i> Add New Complaint Type
                                                            </Button>
                                                        </Link>
                                                        <Button
                                                            type="button"
