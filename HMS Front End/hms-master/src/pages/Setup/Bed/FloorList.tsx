...
const FloorList = () => {
        ...
        },
        {
            ...
            cell: (cell: any) => {
                ...
                const handleAuthorize = () => {
                    ...
                    doApprove(cell.row.original.floorId, newStatus);
                };
                            ...
                            <>
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteFloor(cell.row.original.floorId);
                                        }}
                                        to="#"
                                    >
    ...
    const handleApprove = async () => {
        if (selectedFloorId) {
            try {
                ...
                await setupApiService.approveFloor(selectedFloorId, url);
                toast.success('Floor has been approved Successfully', { containerId: 'TR' });
                await getAllFloor();
    ...
    const editFlore = (data: any) => {
        navigate('/main/editFloor', { state: { data: data } })
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteFloor(selectedId);
    ...
    const getAllFloor = async () => {
        try {
            let result = await setupApiService.getAllFloor();
            setFloorData(result);
        } catch (error: any) {
            return ErrorHandler(error)
        }
                                        ...
                                        <CardHeader className="border-0">
                                            <Row className="g-4 align-items-center">
                                                <div className="col-sm">
                                                    <div>
                                                        <h5 className="card-title mb-0">Floor List</h5>
                                                    </div>
                                                </div>
                                                <div className="col-sm-auto">
                                                    <div>
                                                        <Link to='/main/addFloor'>
                                                            <Button
                                                                size="sm"
                                                                color="primary"
                                                                className="btn btn-primary add-btn me-3"
                                                            >
                                                                <i className="ri-add-fill me-1 align-bottom"></i> Add New Floor
                                                            </Button>
                                                        </Link>
                                                        <Button
                                                            type="button"
