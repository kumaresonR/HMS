...
const ViewTotalIncome = () => {
    ...
    const getAllIncome = async (selectedFilter: any) => {
        try {
            ...
            let ipdIncome = await dashboardApiService.getAllTotalIncome(url);
            setData(ipdIncome)
        } catch (error: any) {
            console.log(error);
            setData([])
                    ...
                    <Col sm={12}>
                        <Card>
                            <CardHeader className="border-0 align-items-center d-flex flex-wrap justify-content-between">
                                <h4 className="card-title mb-0">INCOME</h4>
                                    ...
                                    <Table hover className="table-centered table-bordered align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            {data?.map((data: any, idx: any) => (
                                                <tr key={idx}>
                                                    <td className="text-primary">{data.expenseHead || 'NA'}</td>
                                                    <td className="text-primary">{data.name || 'NA'}</td>
                                            ...
                                            <tr className="table-light">
                                                ...
                                                <td className="text-primary"><b>
                                                    {data?.reduce((total: number, item: any) => total + (parseFloat(item.amount) || 0), 0).toFixed(2)}
                                                </b></td>
                                            </tr>
                                        </tbody>
                                    </Table>
