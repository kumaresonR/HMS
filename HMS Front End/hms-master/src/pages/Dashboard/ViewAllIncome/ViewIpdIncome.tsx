...
const ViewIpdIncome = () => {
    ...
    const getAllIncome = async (selectedFilter: any) => {
        try {
            ...
            let ipdIncome = await dashboardApiService.getAllIPDIncome(url);
            setData(ipdIncome)
        } catch (error: any) {
            console.log(error);
            setData([])
                    ...
                    <Col sm={12}>
                        <Card>
                            <CardHeader className="border-0 align-items-center d-flex flex-wrap justify-content-between">
                                <h4 className="card-title mb-0">IPD INCOME</h4>
                                    ...
                                    <Table hover className="table-centered table-bordered align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            {data?.map((data: any, idx: any) => (
                                                <tr key={idx}>
                                                    <td className="text-primary">{data.ipdId || 'NA'}</td>
