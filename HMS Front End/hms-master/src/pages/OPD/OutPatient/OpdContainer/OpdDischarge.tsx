...
const OpdDischarge = (props: any) => {
    ...
    const doDischarge = async () => {
        try {
            ...
            toast.success('Patient Checkout details updated successfully', { containerId: 'TR' });
            if (props.title==="opd") {
                props.handleClose()
            } 
        } catch (error: any) {
            ...
            <Container fluid>
                <Form onSubmit={handleSubmit}>
                    <Row>
                        <Col>
                            <Alert color="warning">
                            Please ensure the patient's bill is reviewed before proceeding with checkout.
                            </Alert>
                        </Col>
                    </Row>
                    <Row>
                        <Col md={6}>
                            <FormGroup>
                                <label className="text-start mb-2">Checkout Date <span className="text-danger">*</span></label>
                                ...
                                />
                                {dischargeDateValidationError && <div className="invalid-feedback">Checkout Date Required.</div>}
                            </FormGroup>
                        </Col>
                        <Col md={6}>
                            <FormGroup>
                                <label className="text-start mb-2">Checkout Status<span className="text-danger">*</span></label>
                                ...
                                >
                                    <option value="">--Select Checkout Status--</option>
                                    {dischargeStatusData.map((data: any, idx: any) => (
                                        <option key={idx} value={data.status}>{data.status}</option>
                                    ))}
                                </select>
                                {dischargeStatusValidationError && <div className="invalid-feedback">Checkout Status Required.</div>}
                            </FormGroup>
                        </Col>
                        <Col md={12}>
                            <div className="mb-3">
                                <Label className="form-label">Checkout Summary</Label>
                                <textarea
                                    id="note"
                                    name="note"
                                    rows={3}
