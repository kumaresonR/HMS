...
const EditBloodIssue = (props: any) => {
    ...
    const handleBloodGroupChange = (value: any) => {
        ...
        const filteredComponents = componentData.filter((item: any) => item?.donorDetails?.bloodGroup === value);
        if (filteredComponents.length > 0) {
            setFilteredBags(filteredComponents);
        } else {
            setFilteredBags([]);
        }
    };
    ...
    const getAllComponents = async () => {
        try {
            let result = await bloodBankApiService.getAllBagStock();
    ...
    const setBloodIssueData = (data: any) => {
        setSelectedPatientId(data.patientId);
        if (data.patientId) {
            onSearch(data.patientId)
        }
        setIssueDate(moment(data.issueDate).format('YYYY-MM-DDTHH:mm'));
        setHospitalDoctor(data.hospitalDoctor);
        if (data.hospitalDoctor) {
            const firstName = data.hospitalDoctor.split(" ")[0];
        ...
        setBloodQty(data.bloodQty)
        setDocument(data.attachDocument);
        setBag(data?.bagStocks?.bagStockId);
        setBloodGroup(data.bloodGroup);
        if (data.bloodGroup) {
            handleBloodGroupChange(data.bloodGroup);
        }
        setReferenceName(data.referenceName);
        setTechnician(data.technician);
        setChargeCategory(data.chargeCategory);
        if (data.chargeCategory) {
            handleCategoryChange(data.chargeCategory);
        }
        setTotal(data.total);
                ...
                <Row className="g-2">
                    ...
                    <Col>
                        <FormGroup>
                            ...
                            <AsyncTypeahead
                                ...
                                selected={doctorOptions.filter((doctor: any) => doctor.fullName === referenceName)}
                            />
                        </FormGroup>
                    </Col>
                    <Col>
                        ...
                        <FormGroup>
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    setBag(selectedOption);
                                }}
                            >
                                <option value="">--Select--</option>
                                {filteredBags.map((data: any, idx: any) => (
                                    <option key={idx} value={data.bagStockId}>
                                        {data.bagNo} ({data.volume} {data.unitType})
                                    </option>
                                ))}
                            </select>
                    ...
                    <Row className='d-flex justify-content-center'>
                        <Col className="d-flex align-items-center">
                            ...
                            <Button type="submit" disabled={balanceAmount === 0} onClick={handleSubmit}>Save</Button>
                    ...
                    {/* <Col md={4} xs={6}>
                        <FormGroup>
                            ...
                            />
                            {noteError && <div className="invalid-feedback">Note is required.</div>}
                        </FormGroup>
                    </Col> */}
                </Row>
            </Form >
