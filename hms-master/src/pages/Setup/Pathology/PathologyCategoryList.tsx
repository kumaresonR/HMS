...
const PathologyCategoryList = () => {
                            ...
                            <>
                                <li className="list-inline-item">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            editPathology(cell.row.original)
                                        }}
                                        title="Edit"
                                    >
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deletePathology(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedCategoryId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approvePathologyCategory(selectedCategoryId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApprovePathologyCategory(selectedCategoryId);
                    toast.success('Record has been approved UnAuthorized Successfully', { containerId: 'TR' });
                }
                await getAllPathologyCategory();
    ...
    const editPathology = (data: any) => {
        navigate('/main/editPathologyCategory', { state: { data: data } })
    }

    const deletePathology = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deletePathologyCategory(selectedId);
    ...
    const getAllPathologyCategory = async () => {
        try {
            let result = await setupApiService.getAllPathologyCategory();
                    ...
                    <Row className="d-flex justify-content-center">
                        <Col xl={12} className="p-0">
                            <Card>
                                ...
                                <Row>
                                    <Col lg={12}>
                                        <Card id="pathologyCategoryList">
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    <div className="col-sm">
                                                        <div>
                                                            <h5 className="card-title mb-0">Pathology Category List</h5>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addPathologyCategory'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
