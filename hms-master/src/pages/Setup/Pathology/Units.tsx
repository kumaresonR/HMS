...
const UnitList = () => {
                            ...
                            <>
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteUnit(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedUnitId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approvePathologyUnit(selectedUnitId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApprovePathologyUnit(selectedUnitId);
                    toast.success('Record has been approved UnAuthorized Successfully', { containerId: 'TR' });
                }
                await getAllPathologyUnitWorkTable();
    ...
    const editUnit = (data: any) => {
        navigate('/main/editPathologyUnit', { state: { data: data } })
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deletePathologyUnit(selectedId);
    ...
    const getAllPathologyUnitWorkTable = async () => {
        try {
            let result = await setupApiService.getAllPathologyUnitWorkTable();
            setUnitData(result);
        } catch (error: any) {
            return ErrorHandler(error)
        }
                                            ...
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    ...
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addUnitsPathology'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
                                                                    className="btn btn-primary add-btn me-3"
                                                                >
                                                                    <i className="ri-add-fill me-1 align-bottom"></i> Add New Unit
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                type="button"
