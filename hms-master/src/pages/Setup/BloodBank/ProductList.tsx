...
const ProductList = () => {
                            ...
                            <>
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteProduct(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedProductId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveProduct(selectedProductId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveProduct(selectedProductId);
    ...
    const editProduct = (data: any) => {
        navigate('/main/editBloodProducts', { state: { data: data } })
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteProduct(selectedId);
    ...
    const getAllRaiologyParameter = async () => {
        try {
            let result = await setupApiService.getAllProduct();
                                            ...
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    ...
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addBloodProducts'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
                                                                    className="btn btn-primary add-btn me-3"
                                                                >
                                                                    <i className="ri-add-fill me-1 align-bottom"></i> Add New Product
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                type="button"
