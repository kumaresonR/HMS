...
const SupplierList = () => {
    ...
    const supplierColumns = [
        ...
        {
            header: 'Drug License Number',
            accessorKey: 'drugLicenceNumber',
            enableColumnFilter: false,
            cell: (info: any) => (
                <div className=" text-primary">
                    {info.getValue()}
        ...
        // },
        {
            ...
            cell: (cell: any) => {
                ...
                const handleAuthorize = () => {
                    ...
                    doApprove(cell.row.original.supplierId, newStatus);
                };
                            ...
                            <>
                                <li className="list-inline-item">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            editSupplier(cell.row.original)
                                        }}
                                        title="Edit"
                                    >
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteSupplier(cell.row.original.supplierId);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedSupplierId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveSupplier(selectedSupplierId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveSupplier(selectedSupplierId);
    ...
    const editSupplier = (data: any) => {
        navigate('/main/editPharmacySupplier', { state: { data: data } })
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteSupplier(selectedId);
    ...
    const getAllSupplier = async () => {
        try {
            let result = await setupApiService.getAllSupplierTw();
                                            ...
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    ...
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addPharmacySupplier'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
                                                                    className="btn btn-primary add-btn me-3"
                                                                >
                                                                    <i className="ri-add-fill me-1 align-bottom"></i> Add New Supplier
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                type="button"
