...
const MedicineDosageList = () => {
    ...
    const [selectedDosageId, setSelectedDosageId] = useState('');
                            ...
                            <>
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteDosage(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedDosageId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveMedicineDosage(selectedDosageId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveMedicineDosage(selectedDosageId);
    ...
    const editDosage = (data: any) => {
        navigate('/main/editMedicineDosage', { state: { data: data } })
    }

    const deleteDosage = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteMedicineDosage(selectedId);
    ...
    const getAllMedicineDosage = async () => {
        try {
            let result = await setupApiService.getAllMedicineDosageTw();
                    ...
                    <Row className="d-flex justify-content-center">
                        <Col xl={12} className="p-0">
                            <Card>
                                ...
                                <Row>
                                    <Col lg={12}>
                                        <Card id="medicineDosageList">
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    <div className="col-sm">
                                                        <div>
                                                            <h5 className="card-title mb-0">Medicine Dosage List</h5>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addMedicineDosage'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
                                                                    className="btn btn-primary add-btn me-3"
                                                                >
                                                                    <i className="ri-add-fill me-1 align-bottom"></i> Add New Dosage
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                type="button"
