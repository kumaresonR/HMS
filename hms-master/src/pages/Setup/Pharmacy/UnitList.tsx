...
const UnitList = () => {
    ...
    const [selecteUnitId, setSelecteUnitId] = useState('');
        ...
        if (selecteUnitId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveUnit(selecteUnitId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveUnit(selecteUnitId);
    ...
    const editUnit = (data: any) => {
        navigate('/main/editPharmacyUnit', { state: { data: data } })
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteUnit(selectedId);
    ...
    const getAllUnit = async () => {
        try {
            let result = await setupApiService.getAllUnitTw();
            setUnitData(result);
        } catch (error: any) {
            return ErrorHandler(error)
        }
