...
interface FormData {
    serverKey: string; // Changed from username to serverKey
    password: string;
    apiKey: string;
    statusVal: string;
}
...
const MidtransForm: React.FC = () => {
    const [formData, setFormData] = useState<FormData>({
        serverKey: '', // Initialize serverKey
        password: '',
        apiKey: '',
        statusVal: '',
    });
    ...
    const validateForm = (data: FormData): FormErrors => {
        ...
        if (!data.serverKey) newErrors.serverKey = 'Server Key is required'; // Validate serverKey
    ...
    return (
        <Form onSubmit={handleSubmit}>
            <FormGroup>
                <Label for="serverKey">Server Key</Label> {/* Changed label to Server Key */}
                <Input
                    type="text"
                    id="serverKey" // Changed id to serverKey
                    value={formData.serverKey}
                    onChange={handleChange}
                    invalid={!!errors.serverKey} // Validate for serverKey
                />
                {errors.serverKey && <div className="invalid-feedback">{errors.serverKey}</div>}
            </FormGroup>
