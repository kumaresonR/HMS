...
const IncomeHeadList = () => {
                            ...
                            <>
                                <li className="list-inline-item">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            editIncomeHead(cell.row.original)
                                        }}
                                        title="Edit"
                                    >
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteIncomehead(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedCategoryId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveIncomeHeads(selectedCategoryId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveIncomeHeads(selectedCategoryId);
                    toast.success('Record has been UnAuthorized Successfully', { containerId: 'TR' });
                }
                await getAllIncomeHeads();
    ...
    const editIncomeHead = (data: any) => {
        navigate('/main/editIncomeHead', { state: { data: data } })
    }

    const deleteIncomehead = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteIncomeHeads(selectedId);
    ...
    const getAllIncomeHeads = async () => {
        try {
            let result = await setupApiService.getAllIncomeHeadsTw();
                    ...
                    <Row className="d-flex justify-content-center">
                        <Col xl={12} className="p-0">
                            <Card>
                                ...
                                <Row>
                                    <Col lg={12}>
                                        <Card id="incomeHeadList">
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    <div className="col-sm">
                                                        <div>
                                                            <h5 className="card-title mb-0">Income Head List</h5>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addIncomeHead'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
                                                                    className="btn btn-primary add-btn me-3"
                                                                >
                                                                    <i className="ri-add-fill me-1 align-bottom"></i> Add New Income Head
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                type="button"
