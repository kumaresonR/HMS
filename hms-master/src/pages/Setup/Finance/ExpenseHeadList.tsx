...
const ExpenseHeadList = () => {
                            ...
                            <>
                                <li className="list-inline-item">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            editExpenceHead(cell.row.original)
                                        }}
                                        title="Edit"
                                    >
                                ...
                                <li className="list-inline-item" title="Delete">
                                    <Link
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            deleteExpencehead(cell.row.original.id);
                                        }}
                                        to="#"
                                    >
        ...
        if (selectedCategoryId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveExpenseHeads(selectedCategoryId);
                    toast.success('Record has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveExpenseHeads(selectedCategoryId);
                    toast.success('Record has been UnAuthorized Successfully', { containerId: 'TR' });
                }
                await getAllExpenseHeads();
    ...
    const editExpenceHead = (data: any) => {
        navigate('/main/editExpenseHead', { state: { data: data } })
    }

    const deleteExpencehead = (id: any) => {
        setSelectedId(id);
        setDeleteModal(true);
    }
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteExpenseHeads(selectedId);
    ...
    const getAllExpenseHeads = async () => {
        try {
            let result = await setupApiService.getAllExpenseHeadsTw();
            setExpenseHeadData(result);
        } catch (error: any) {
            return ErrorHandler(error)
        }
    }
                    ...
                    <Row className="d-flex justify-content-center">
                        <Col xl={12} className="p-0">
                            <Card>
                                ...
                                <Row>
                                    <Col lg={12}>
                                        <Card id="expenseHeadList">
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    <div className="col-sm">
                                                        <div>
                                                            <h5 className="card-title mb-0">Expense Head List</h5>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addExpenseHead'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
                                                                    className="btn btn-primary add-btn me-3"
                                                                >
                                                                    <i className="ri-add-fill me-1 align-bottom"></i> Add New Expense Head
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                type="button"
