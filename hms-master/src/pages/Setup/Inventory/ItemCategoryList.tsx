...
const ItemCategoryList = () => {
    ...
    const getAllInventoryCategory = async () => {
        try {
            let result = await setupApiService.getAllInventoryCategoryTw();
    ...
    const editDetails = (id: any) => {
        navigate('/main/editItemCategorySetup', { state: { data: id } })
    }
        ...
        if (selectedCategoryId) {
            ...
            try {
                if (status === "AUTHORIZED") {
                    await setupApiService.approveInventoryCategory(selectedCategoryId);
                    toast.success('Item Category has been Authorized Successfully', { containerId: 'TR' });
                } else {
                    await setupApiService.disApproveInventoryCategory(selectedCategoryId);
                    toast.success('Item Category has been UnAuthorized Successfully', { containerId: 'TR' });
                }
                await getAllInventoryCategory();
    ...
    const handleDelete = async () => {
        if (selectedId) {
            try {
                await setupApiService.deleteInventoryCategory(selectedId);
                    ...
                    <Row className="d-flex justify-content-center">
                        <Col xl={12} className="p-0">
                            <Card>
                                ...
                                <Row>
                                    <Col lg={12}>
                                        <Card id="itemCategoryList">
                                            <CardHeader className="border-0">
                                                <Row className="g-4 align-items-center">
                                                    <div className="col-sm">
                                                        <div>
                                                            <h5 className="card-title mb-0">Item Category List</h5>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-auto">
                                                        <div>
                                                            <Link to='/main/addItemCategorySetup'>
                                                                <Button
                                                                    size="sm"
                                                                    color="primary"
