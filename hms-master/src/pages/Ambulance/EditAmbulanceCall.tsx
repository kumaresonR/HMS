...
const EditAmbulanceCall = (props: any) => {
    ...
    const [initialPaymentAmount, setInitialPaymentAmount] = useState<number | null>(null);
    ...
    const handlePaymentAmountChange = (value: any) => {
        ...
        if (balanceAmount !== 0) {
            if (numericValue > netAmount || !numericValue) {
    ...
    const setAmbulanceCallData = (data: any) => {
        ...
        if (data.vehicleId && ambulanceData.length > 0) {
            handleVehicleModelChange(data.vehicleId);
        }

        setVehicleModel(data.vehicleModel)
        setDriverName(data.driverName);
        setDate(moment(data?.transactions[0]?.date).format('YYYY-MM-DDTHH:mm'));
        ...
        setPaymentMode(data.transactions[0].paymentMode);
        setChequeDate(data.transactions[0].checkDate);
        setChequeNo(data.transactions[0].checkNo);
        setDocument(data.transactions[0].attachment);
        setNetAmount(data.netAmount)
        setInitialPaymentAmount(data.balanceAmount);
        setPaymentAmount(data.balanceAmount);
        setBalanceAmount(data.balanceAmount);
        setIpdOrOpdId(data.ipdOrOpdId);
    }
    ...
    const validateForm = () => {
        ...
        if (balanceAmount !== 0 && (!paymentAmount || parseFloat(paymentAmount) > netAmount)) {
            setPaymentAmountValidationError(true);
            isFormValid = false;
        } else {
            setPaymentAmountValidationError(false);
    ...
    const doCreatePayment = async () => {
        try {
            ...
            await ambulanceApiService.editAmbulanceCall(id.vehicleChargeId, formData);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (props.data) {
                setAmbulanceCallData(props.data);
            }
        };
        fetchData();
    }, []);
    ...
    useEffect(() => {
        if (total !== initialPaymentAmount && discount !== 0) {
            recalculateNetAmount(total, discountPercent, taxPercentage);
        } else {
            setPaymentAmount(initialPaymentAmount);
        }
    }, [total, discount, taxPercentage]);
                                        ...
                                        <form onSubmit={handleSubmit}>
                                            <div className="around10">
                                                        ...
                                                        <Row>
                                                            ...
                                                            <Col sm={6}>
                                                                <FormGroup>
                                                                    ...
                                                                    <small className="mx-2 text-success">Balance {balanceAmount}</small>
                                                                    <Input
                                                ...
                                                <div className="col-md-12 text-right">
                                                    ...
                                                    >
                                                        {loading ? <Spinner size="sm" /> : 'Update Ambulance Call'}
                                                    </Button>
                                                </div>
...
export default EditAmbulanceCall