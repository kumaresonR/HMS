...
const BloodIssueReport = () => {
    ...
    // total 
    const totalAmount = bloodIssueData.reduce((sum: any, donor: any) => sum + (donor.netAmount || 0), 0);
    const totalPaidAmount = bloodIssueData.reduce((sum: any, donor: any) => sum + (donor.paymentAmount || 0), 0);
    const totalBalanceAmount = totalAmount - totalPaidAmount;

    const bloodIssueColumns = [
        {
            header: 'Bill No',
            accessorKey: 'issueBloodId',
    ...
    const getAllPatientData = async () => {
        try {
            ...
            let result = await reportApiService.getAllBloodIssueReport(url);
            setBloodIssueData(result.content);
    ...
    return (
        <React.Fragment>
            <div>
                <h5 className="card-title mb-4">Blood Issue Report</h5>
                <Row>
                    <Col lg>
                        <Row>
                            <Col lg={4}>
                ...
                <Row>
                    <Col lg={12} className="px-0">
                        <Card id="bloodIssueReport">
                            <CardHeader className="border-0">

                            </CardHeader>
                            <div className="card-body pt-0">
                                <div className="col-sm-auto">
                                    {bloodIssueData?.length > 0 && (
