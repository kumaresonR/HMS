...
import PostalApiService from '../../helpers/services/employee/EmployeeApiService';
import { minimizePage } from '../../slices/pageResizer/uiSlice';
import { useDispatch } from 'react-redux';

const AddReceiver: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    //   if (location.state?.data) {
    //     await postalApiService.updateReceiver(location.state.data.id, formData);
    //     toast.success('Receiver updated successfully', { containerId: 'TR' });
    //   } else {
    //     await postalApiService.createReceiver(formData);
    //     toast.success('Receiver created successfully', { containerId: 'TR' });
    //   }

      navigate('/main/postalReceiverList');
    } catch (err: any) {
      ErrorHandler(err);
    }
  };
  ...
  return (
    <Container fluid>
      <FormHeader
        title={location.state?.data ? 'Edit Receiver' : 'Add Receiver'}
        pageTitle="Front Office"
        onMinimize={() => dispatch(minimizePage({ route: location.pathname, pageName: location.state?.data ? 'Edit Receiver' : 'Add Receiver' }))}
      />
      <Card>
        <CardBody>
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h5 className="mb-0">{location.state?.data ? 'Edit Receiver' : 'Add Receiver'}</h5>
            <Link to="/main/postalReceiverList">
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Button color="primary" type="submit">
              {location.state?.data ? 'Update Receiver' : 'Add Receiver'}
            </Button>
          </Form>
        </CardBody>
      </Card>
