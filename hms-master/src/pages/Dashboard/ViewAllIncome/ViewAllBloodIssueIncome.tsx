...
const ViewAllBloodIssueIncome = () => {
    ...
    const getAllIncome = async (selectedFilter: any) => {
        try {
            ...
            let ipdIncome = await dashboardApiService.getAllBloodBankIssueIncome(url);
            setData(ipdIncome)
        } catch (error: any) {
            console.log(error);
            setData([])
    ...
    return (
        <React.Fragment>
            <Container fluid>
                <FormHeader
                    title="BLOOD COMPONENT ISSUE INCOME"
                    pageTitle="Billing"
                    onMinimize={() => dispatch(minimizePage({
                        route: location.pathname,
                        pageName: "BLOOD COMPONENT ISSUE INCOME",
                    }))} />
                <Row>
                    <Col sm={12}>
                        <Card>
                            <CardHeader className="border-0 align-items-center d-flex flex-wrap justify-content-between">
                                <h4 className="card-title mb-0">BLOOD COMPONENT ISSUE INCOME</h4>
                                    ...
                                    <Table hover className="table-centered table-bordered align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            {data?.map((data: any, idx: any) => (
                                                <tr key={idx}>
                                                    <td className="text-primary">{data.issueComponentId || 'NA'}</td>
                                            ...
                                            <tr className="table-light">
                                                <td colSpan={5}></td>
                                                <td colSpan={2}><b>
                                                    Total Standard Charge: {data?.reduce((acc: number, item: any) => acc + (parseFloat(item.standardCharge) || 0), 0).toFixed(2)}
                                                </b></td>
                                                <td><b>
                                                    {data?.reduce((acc: number, item: any) => acc + (parseFloat(item.total) || 0), 0).toFixed(2)}
                                                </b></td>
                                                <td><b>
                                                    {data?.reduce((acc: number, item: any) => acc + (parseFloat(item.discount) || 0), 0).toFixed(2)}
                                                </b></td>
                                                <td><b>
                                                    {data?.reduce((acc: number, item: any) => acc + (parseFloat(item.tax) || 0), 0).toFixed(2)}
                                                </b></td>
                                                <td><b>
                                                    {data?.reduce((acc: number, item: any) => acc + (parseFloat(item.netAmount) || 0), 0).toFixed(2)}
                                                </b></td>
                                                <td><b>
                                                    {data?.reduce((acc: number, item: any) => acc + (parseFloat(item.paymentAmount) || 0), 0).toFixed(2)}
                                                </b></td>
                                                <td><b>
                                                    {data?.reduce((acc: number, item: any) => acc + (parseFloat(item.balanceAmount) || 0), 0).toFixed(2)}
                                                </b></td>
                                            </tr>
                                        </tbody>
                                    </Table>
...
export default ViewAllBloodIssueIncome