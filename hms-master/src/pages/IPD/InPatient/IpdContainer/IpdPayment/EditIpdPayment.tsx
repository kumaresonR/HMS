...
const EditIpdPayment = (props: any) => {
    ...
    const getPaymentById = async () => {
        try {
            if (props.title === 'ipd') {
                let data = await iPDApiService.getPaymentById(props.id);
                setData(data);
            } else {
                let data = await opdApiService.getPaymentById(props.id);
                setData(data);
            }
        } catch (error) {
            console.log(error);
    ...
    const doCreatePayment = async () => {
        try {
            ...
            if (props.title === 'ipd') {
                await iPDApiService.editPayment(props.id, formData);
            } else {
                await opdApiService.editPayment(props.id, formData);
            }

            toast.success('Payment Updated Successfully', { containerId: 'TR' });
            props.handleClose();
            props.refresh();
        } catch (error: any) {
