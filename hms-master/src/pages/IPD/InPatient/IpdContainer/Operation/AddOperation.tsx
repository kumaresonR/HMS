...
const AddOperation = (props: any) => {
    ...
    const [consultantDoctorValidationError, setOperationNameValidationError] = useState(false);
    const [operationDate, setOperationDate] = useState('');
    const [operationNameValidationError, setOperationDateValidationError] = useState(false);
    const [consultantDoctor, setConsultantDoctor] = useState('');
    const [operationDateValidationError, setConsultantDoctorValidationError] = useState(false);
    ...
    const doCreateOperation = async () => {
        try {
            ...
            if(props.title === 'ipd') {
                await ipdApiService.createOperation(payload);
            } else {
                await opdApiService.createOperation(payload);
            }
            
            toast.success('Operation Created Successfully', { containerId: 'TR' });
            props.handleClose();
            props.refresh();
        } catch (error: any) {
