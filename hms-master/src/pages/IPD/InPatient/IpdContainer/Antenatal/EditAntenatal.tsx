...
const EditAntenatal = (props: any) => {
    ...
    const [antenatalWeight, setAntenatalWeight] = useState('');
    ...
    const doCreateOperation = async () => {
        try {
            ...
            await ipdApiService.editAntenatal(props.data.antenatalId,payload);
            toast.success('Antenatal Updated Successfully', { containerId: 'TR' });
            props.handleClose();
            props.refresh()
        } catch (error: any) {
    ...
    const setAntenatalData = (data: any) => {
        setIpdId(props.ipdId);
        setBleeding(data.bleeding);
        setHeadache(data.headache);
        setPain(data.pain);
        setConstipation(data.constipation);
        setUrinarySymptoms(data.urinarySymptoms);
        setVomiting(data.vomiting);
        setCough(data.cough);
        setVaginal(data.vaginal);
        setDischarge(data.discharge);
        setHeight(data.height);
        setOedema(data.oedema);
        setHaemoroids(data.haemoroids);
        setWeight(data.weight);
        setDate(moment(data.date).format('YYYY-MM-DD'));
        setCondition(data.condition);
        setSpecialFindingsAndRemark(data.specialFindingsAndRemark);
        setPelvicExamination(data.pelvicExamination);
        setSP(data.sp);
        setUterSize(data.uterSize);
        setUterusSize(data.uterusSize);
        setPresentationPosition(data.presentationPosition);
        setPresentingPartToBrim(data.presentingPartToBrim);
        setFoetaHeart(data.foetalHeart);
        setBloodPressure(data.bloodPressure);
        setAntenatalOedema(data.antenatalOedema);

        setUrine(data.urineSugar);
        setUrineAaibumen(data.urineAlbumen);
        setAntenatalWeight(data.antenatalWeight);
        setRemark(data.remark);
        setNextVisit(moment(data.nextVisit).format('YYYY-MM-DD'));
        setPreviousAntenatalDetails(data.previousAntenatalDetails);
    }

    useEffect(() => {
        setAntenatalData(props.data);
    }, []);
                            ...
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <Input
                                        ...
                                        onChange={e => setAntenatalWeight(e.target.value)}
                                    />
                                </FormGroup>
                            </Col>
                            <Col md={12}>
