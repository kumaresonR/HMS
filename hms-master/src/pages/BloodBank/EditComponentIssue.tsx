...
const EditComponentIssue = (props: any) => {
    ...
    const getAllComponentIssue = async () => {
        try {
            let id = props.data.issueComponentId
            let result = await bloodBankApiService.getAllComponentIssue(id);
            setPathologyIssueData(result);
        } catch (error: any) {
            console.log(error);
        }
    }

    const setPathologyIssueData = (data: any) => {
        ...
        setBag(data.bloodComponent.componentId);
        ...
        if (data.components) {
            handleComponentChange(data.components);
        }
    ...
    const doCreateIssueComponent = async () => {
        try {
            ...
            await bloodBankApiService.editComponentIssue(props.data.issueComponentId, formData);
    ...
    useEffect(() => {
        if (filteredCategory.length > 0 && components) {
            const filteredBags = filteredCategory.filter((item: any) => item.componentName === components);
            if (filteredBags.length > 0) {
                setFilteredBags(filteredBags);
            } else {
                setFilteredBags([]);
                            ...
                            <FormGroup>
                                ...
                                >
                                    ...
                                    {filteredBags.map((data: any, idx: any) => (
                                        <option key={idx} value={data.componentId}>{data.componentBag}</option>
                                    ))}
                                </select>
...
export default EditComponentIssue