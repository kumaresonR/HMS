...
const RadiologyBill = (props: any) => {
        ...
        setRows((prevRows) => {
            ...
            const updatedRows = prevRows.map((row) => {
                const test = pathologyTestData.find(
                    (t:any) => t.radiologyTestId === row.radiologyTestId
                );
    ...
    const getAllPrescriptions = async (id: any) => {
        try {
            ...
            const combinedData = [...ipdData, ...opdData].filter(item => !item.radiologyPaid);
        ...
        try {
            if (prescriptionNo) {
                ...
                if (prescriptionNo.startsWith("IPDP")) {
                    ...
                    prescriptionDetails = ipdData?.[0]?.radiologyId || [];
                    ...
                    prescriptionDetails = opdData?.[0]?.radiologyId || [];
                ...
                prescriptionDetails.forEach((radiologyTestId: any, index: any) => {
                    handleTestNameChange(index, radiologyTestId);
                });
            } else {
    ...
    const handleInputChange = (index: number, field: string, value: string) => {
        setRows((prevRows) => {
            ...
            // Aggregate totals and tax in rupees
            let total = 0;
            let totalTaxAmount = 0; // Tax in rupees
            ...
            setTotal(total.toFixed(2));
            setTax(totalTaxAmount.toFixed(2)); // Set total tax in rupees

            // Calculate Net Amount after discount and including totalTaxAmount
            const discountValue = total * (discount / 100);
            const netAmount = total - discountValue + totalTaxAmount; // Add tax amount to total after discount
            setNetAmount(netAmount.toFixed(2));
            setPaymentAmount(netAmount.toFixed(2));
    ...
    const doCreatePayment = async () => {
        try {
            ...
            await billingApiService.createRadiologyBill(formData);
    ...
    return (
        <Container fluid>
            ...
            <div className=' row d-flex justify-content-between'>
                <div className="col-5">    <h4>Add Details</h4></div>
                <div className="col-6 text-end">
                    ...
                    {/* <Button color="success" onClick={addRow} disabled>
                    Add
                </Button> */}
                </div>
            ...
            <Form onSubmit={handleSubmit}>
                {rows.map((row, index) => (
                    <Row key={index} className="g-4 align-items-center">
                        <Col  >
                            <FormGroup>
                        ...
                        {/* <Col sm={1}>
                            {index !== 0 && (
                                <button className='btn btn-soft-danger' onClick={() => removeRow(index)}>x</button>
                            )}
                        </Col> */}
