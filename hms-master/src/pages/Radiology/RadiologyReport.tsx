...
const RadiologyReport = (props: any) => {
    ...
    // const onFileUploadListener = async (event: any) => {
    //     const file = event.target.files[0];
    //     if (!file) return;
    ...
    //     // Optional: Check if under 10KB
    //     if (compressedBlob.size > 10240) {
    //         alert('Compressed file is still larger than 10KB');
    //         return;
    //     }
    
    //     setDocument(compressedBlob);
    // };
    ...
    const getRadiologyByTestId = async () => {
        try {
            let url = props.data.testId;
            let result = await billingApiService.getRadiologyByTestId(url);
            setRadiologydata(result.radiologyItems);
            console.log(result.radiologyItems);
            if (result.radiologyItems && result.radiologyItems.length > 0) {
                // Pass the first item of radiologyItems to setData
                setData(result.radiologyItems[0]);
            }
            const testParameters = result.radiologyItems
                .flatMap((item: any) => item.radiologyTestDetails?.testParameters || []);
            setTestParameterData(testParameters);

        } catch (error: any) {
            console.error("Error in getRadiologyByTestId:", error);
            return ErrorHandler(error);
        }
    };
    ...
    const setData = (data: any) => {
        ...
        const testParameters = data.radiologyTestDetails?.testParameters || [];
        setTestParameterData(testParameters);
        setUploadedImg(data.uploadReport)
    ...
    const validateForm = () => {
        ...
        //     if (!row.reportValue || row.reportValue.trim() === '') {
        //         errors[index] = true;
        //         isFormValid = false;
        //     }
        // });
    ...
    const doCreateLabReport = async () => {
        try {
            ...
            await billingApiService.approveRadiologyReport(props.data.testId, formData);
                ...
                <Row>
                    <Col>
                        <Row>
                            ...
                            <Col><Label>{data?.radiologyDetails?.testName || 'NA'}</Label></Col>
                        </Row>
                        <Row>
                    ...
                    <Col>
                        ...
                        <Row>
                            <Col><Label>Radiology Center</Label></Col>
                            <Col><Label>{data.radiologyCenter || 'NA'}</Label></Col>
                        </Row>
                    </Col>
                </Row>
                <Row>
