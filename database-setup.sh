#!/bin/bash
# =====================================================
# HMS Database Setup Script for Linux/Mac
# =====================================================
# This script creates the database and schemas for local development
#
# Prerequisites:
# 1. PostgreSQL must be installed
# 2. psql must be in your PATH
# 3. Default password: Test@123 (or update the script)
# =====================================================

echo "====================================================="
echo "HMS Database Setup for Local Development"
echo "====================================================="
echo ""

# Check if PostgreSQL is installed
if ! command -v psql &> /dev/null; then
    echo "ERROR: PostgreSQL psql command not found!"
    echo "Please ensure PostgreSQL is installed and added to PATH"
    exit 1
fi

echo "Step 1: Creating database and schemas..."
echo ""

# Set PostgreSQL password (update if different)
export PGPASSWORD=Test@123

# Run the SQL script
psql -U postgres -f database-setup.sql

if [ $? -eq 0 ]; then
    echo ""
    echo "====================================================="
    echo "Database setup completed successfully!"
    echo "====================================================="
    echo ""
    echo "Database: myappdb"
    echo "Username: postgres"
    echo "Password: Test@123"
    echo ""
    echo "All schemas have been created."
    echo "You can now start the HMS services."
    echo ""
else
    echo ""
    echo "====================================================="
    echo "ERROR: Database setup failed!"
    echo "====================================================="
    echo ""
    echo "Please check:"
    echo "1. PostgreSQL is running"
    echo "2. Password is correct (currently: Test@123)"
    echo "3. You have superuser privileges"
    echo ""
    exit 1
fi

